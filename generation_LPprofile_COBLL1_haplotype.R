...
###load LP data set#####
combined_data<-read.csv(file = "raw_data_LP_PAC_SNP.csv")
combined_data<-combined_data[, -1]

#####subset condition - free fatty acid or without###
...
########day 8 has fewest individuals ### define samples that have data for alle timepoints####
patient_overlap<-as.character(data_nonFFA_day8_sc$patientID)

#############input data you want to analyse##########
AP_subset<-data_nonFFA_day14_sc
####subset dataframe so that all timepoints have same samples#####
AP_subset<-subset(AP_subset, AP_subset$patientID %in% patient_overlap)

#####subset in meta and LP feature data######
meta<-AP_subset[, 1:12]
input<-AP_subset[, 13:1195]

#####filter data, remove features with 0´s or NA´s across all samples#####
...
case<-subset(df, df$rs6712203== "CC")

control<-subset(df, df$rs6712203  == "TT")

#####merge case and control###
data_anov<-rbind(case, control)

####transform LP features to numeric and change range depending on input####
data_anov[,13:1171]<- sapply(sapply(data_anov[,13:1171], as.character),as.numeric) 
...
pdf("risk vs non-risk COBLL1 profile subq D14 volcano plot.pdf")
plot
dev.off()

######subset depending on which anylsis you want to run
write.table(volcano, file = "risk vs non-risk COBLL1 profile subq D14.csv", sep = ",", col.names = NA,
            qmethod = "double")
